if object_id('fn_Drew_Restore_MedPeople_DiffTable_t') is not null
	drop function fn_Drew_Restore_MedPeople_DiffTable_t
go

create function fn_Drew_Restore_MedPeople_DiffTable_t()
...
as begin
	...
	('LinkCredentialsToJobOrders', N'WHERE LinkPeopleToCredentialsID in(select LinkPeopleToCredentialsID from <DB>..LinkPeopleToCredentials where PeopleID = @MainRecordID)'),
	...
	('UsersCommissionsSplit', N'WHERE Type = ''Placement'' and ObjectID in(select PositionsID from <DB>..Positions where PeopleID = @MainRecordID and isnull(ProjectsID, 0) = 0 and isnull(JobOrdersID, 0) = 0)'),
	...
	('Interview', N'WHERE InterviewID in(select Interview.InterviewID from <DB>..Interview join <DB>..ProjectsClientTeams pct on pct.PeopleID = Interview.Interviewer and pct.ProjectsID = Interview.ProjectsID and Interview.Done = 0 and Interview.PeopleID = @MainRecordID)'),
	('UsersCommissionsSplit', N'WHERE Type = ''Interview'' and ObjectID in(select Interview.InterviewID from <DB>..Interview join <DB>..ProjectsClientTeams pct on pct.PeopleID = Interview.Interviewer and pct.ProjectsID = Interview.ProjectsID and Interview.Done = 0 and Interview.PeopleID = @MainRecordID)'),
	('Task', N'WHERE TaskID in(select Interview.TaskID from <DB>..Interview join <DB>..ProjectsClientTeams pct on pct.PeopleID = Interview.Interviewer and pct.ProjectsID = Interview.ProjectsID and Interview.Done = 0 and Interview.PeopleID = @MainRecordID)'),
